...
# `class Task:` - определение класса Task.)

# `def __init__&#40;self, description, deadline&#41;:` - определение конструктора класса Task с параметрами description и deadline.)

# `self.description = description` - присваивание переданного значения описания задачи атрибуту description объекта Task.)

#  `self.deadline = deadline` - присваивание переданного значения срока выполнения задачи атрибуту deadline объекта Task.)

#  `self.status = "Not done"` - установка начального статуса задачи как "Не выполнено".)

#  `def mark_as_done&#40;self&#41;:` - определение метода mark_as_done для отметки задачи как выполненной.)

#  `self.status = "Done"` - установка статуса задачи как "Выполнено".)
# 
# (По аналогии, строки в классе Store могут быть разобраны следующим образом:)
